  ...
  return (
    <main className="min-h-screen bg-primary-white overflow-x-hidden">
      ...
      <section className="relative min-h-screen w-full bg-[#F5F5F5] overflow-hidden">
        ...
        {/* Desktop - GRID OVERLAP TECHNIQUE */}
        <div className="hidden lg:grid min-h-screen max-w-7xl mx-auto px-8">
          <div className="grid grid-cols-1 min-h-screen">
            {/* Layer 1: Text - same cell */}
            <div className="col-start-1 row-start-1 flex items-center z-10">
              <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} className="w-[55%] space-y-8 py-32">
                <p className="text-sm uppercase tracking-widest text-gray-500">Creative Director & Brand Strategist</p>
            ...
            {/* Layer 2: Image - SAME cell, higher z-index */}
            <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }} className="col-start-1 row-start-1 flex items-end justify-end z-20 pointer-events-none">
      ...
      <section className="bg-green-50 py-6 px-6">
        <p className="text-center text-gray-700">
          <strong>⭐ Recommended!</strong> Uses CSS Grid so both elements share the same stacking context - z-index works reliably.
        </p>
      </section>
    </main>
  )
